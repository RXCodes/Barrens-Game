	...
	if Player.current != null:
		...
		while not dead:
			...
			while withinRangeOfTarget():
				...
				playAnimation("BodySlam")
				# Use the hurtbox to deal damage to the player
				var damage = randf_range(15, 25) # random float from 15 to 25
...
func _process(delta: float) -> void:
	if current_state == State.DEAD:
		return  # Skip the rest if the worm is dead

	if Player.current != null and collisionRigidBody != null:
		is_player_in_range = (Player.current.global_position - collisionRigidBody.global_position).length() <= 100
...
func move_towards_player(delta: float) -> void:
	if Player.current != null and collisionRigidBody != null:
		...
		if (Player.current.global_position - collisionRigidBody.global_position).length() <= 20:
...
func attack_player() -> void:
	...
	if Player.current != null:
		Player.current.take_damage(30)  # Adjust damage value as needed
