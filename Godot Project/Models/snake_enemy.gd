	...
	if Player.current != null:
		...
		while not dead:
			...
			while withinRangeOfTarget():
				...
				var damage = randf_range(6, 12) # random float from 6 to 12
...
func _process(delta: float) -> void:
	if current_state == State.DEAD:
		return  # Skip the rest if the snake is dead

	if Player.current != null and collisionRigidBody != null:
		is_player_in_range = (Player.current.global_position - collisionRigidBody.global_position).length() <= 50
...
func attack_player() -> void:
	...
	if Player.current != null:
		Player.current.take_damage(18)  # Adjust damage value as needed
...
func kill() -> void:
	...
	# drop_cash()
	queue_free()  # Remove the snake from the scene
